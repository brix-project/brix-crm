...
class CrmCustomerWrapper
{
    ...
    public function getInvoiceTemplate(): T_CRM_Invoice
    {
        $template = $this->customerDir->withRelativePath("invoice-tpl.yml");
        if (!$template->isFile()) {
            throw new \InvalidArgumentException("Customer has no invoice-tpl.yml");
        ...
        return $template->assertFile()->get_yaml(T_CRM_Invoice::class);
    }

    public function createAiInvoiceCreator(T_CRM_Invoice $originalInvoice): AiInvoiceCreator
    {
        return new AiInvoiceCreator(
            $originalInvoice,
   ...
   public function getOffer(string $offerId) : T_CRM_Invoice {
    ...
    public function buildInvoice (T_CRM_Invoice $invoice) : string {
        ...
        $layout = $this->brixEnv->rootDir
            ->withRelativePath($tentant->invoice_layout)
            ->assertFile()
            ->get_yaml(T_Layout::class);
        if (!$layout instanceof T_Layout) {
            throw new \UnexpectedValueException("Invalid invoice layout: " . $tentant->invoice_layout);
        }
